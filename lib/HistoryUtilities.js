...
History.prototype.getMessagesBy = function(username, callback){
	this.db.collection("Group" + this.groupID + "Messages").find({name: username}, function(error, response){
		callback(error, response);
	});
}
 ...
 * callback 		- callback, returns all errors or array of user_ids
 */
HistoryUtilities.prototype.findUserID = function(username, callback){
	this.history.getMessagesBy(username, function(error, response){
		if(!error && response.length > 0){
			var user_ids = [];
			for(var i in response){
				if(user_ids.indexOf(response[i].user_id) == -1){
 ...
 * callback 		- callback, returns all errors or array of user_ids
 */
HistoryUtilities.prototype.getMessagesBy = function(username, callback){
	this.history.getMessagesBy(username, function(error, response){
		callback(error, response);
	});
}
