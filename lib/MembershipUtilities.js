...
Memberships.prototype.transferMembers = function(fromGroup, toGroup, callback){
	...
	this.getMembers(fromGroup, toGroup, function(error, response){
		if (!error || error.length == 0 && response) {
			uniqueMembers = [];
			uniqueMembers = response["toAddList"];

			console.log("\033[93m" + uniqueMembers  + "\033[0m");
...
Memberships.prototype.bindMembers = function(fromGroup, fromInclusive, toGroup, toInclusive, callback){
	...
	var modifyRosters = function(memberGroups){
		...
		var add = function(list, group){
			if(list.length == 0){
				console.log("\033[93mNo Users To Add in Group " + group + "\033[0m");
				responses.push("No Users to Add in Group " + group);
				callbackCount--;	
				if(callbackCount == 0){
		...
		var remove = function(list, group){
			if(list.length == 0){
				console.log("\033[93mNo Users To Remove in Group " + group + "\033[0m");
				responses.push("No Users to Remove in Group " + group);
				callbackCount--;	
				if(callbackCount == 0){
					callback(errors, responses);
				}
 ...
 *
 * memberList		- Array, containg user_ids of members to be added to the group
...
Memberships.prototype.removeMembers = function(memberList, groupID, callback){
	...
	var removeCall = function(memberObject, callback){
		...
		API.Members.remove(
			...
			function(error, response) {
				if(!error || error.length == 0 && response) {
					console.log("\033[93m"+ memberObject.nickname + " Deleted (id: " + memberObject.id + ", user_id: " + memberObject.user_id + ")\033[0m");
 ...
 *
 * memberList		- Array, containg user_ids of members to be messaged
...
/**
 * MembershipUtilities gatherRoster function
 *
 * Sends a Get request to GroupMe API to gather and return an array of all group members from GROUP_ID
 *
 * GROUP_ID			- String, ID for the group whose members are to be retrieved
 * callback 		- callback, returns all members from group with GROUP_ID along with any errors
 */
MembershipUtilities.gatherRoster = function(ACCESS_TOKEN, GROUP_ID, callback) {
	var memberships = new Memberships(ACCESS_TOKEN);
	memberships.gatherRoster(GROUP_ID, function(error, response){
 ...
 *
 * ACCESS_TOKEN		- String, personal access token for GroupMe API
 * MEMBER_LIST		- Array, containg user_ids of members to be added to the group
 ...
 *
 * ACCESS_TOKEN		- String, personal access token for GroupMe API
 * MEMBER_LIST		- Array, containg user_ids of members to be messaged
